import { CountryProgram } from "@/types/program"
import { CenterItem } from "@/types/program";


export const ourCentersData: Record<string, {
  country: string;
  centers: CenterItem[];
}>  = {
  philippines: {
    country: "philippines",
    centers: [
      {
        schools: [
          "Don Bosco Academy of Pampanga",
          "Don Bosco Training Center of Naga, Inc",
          "Tuloy Foundation, Inc",
          "Don Bosco Technical College",
          "Don Bosco TVET Center – San Jose, Nueva Ecija",
          "Don Bosco TVET Center – Calauan Inc.",
          "Don Bosco Agro-Mechanical Technology Center (Legazpi), Inc",
          "Don Bosco Technical Institute of Makati, Inc",
          "Don Bosco Youth Center – Tondo Inc.",
          "Don Bosco College, Inc",
          "Don Bosco Technical Vocational Education and Training Center – Pasil, Inc",
          "Don Bosco Boys Home, Inc",
          "Don Bosco Technical College – Cebu, Inc",
          "Don Bosco Technical Vocational Education and Training Center – Balamban, Inc",
          "Don Bosco Boys Home and Vocational Training Center, Inc",
          "Don Bosco Technical Institute of Victorias, Inc",
          "Don Bosco Training Center – Mati",
          "Don Bosco Training Center – Borongan, Inc",
        ],
        image: "/philliphines-map.png",
      },
    ],
  },

  indonesia: {
    country: "indonesia",
    centers: [
      {
        schools: [
          "Don Bosco Technical School – Jakarta",
          "Don Bosco Training Center – Surabaya",
          "Don Bosco Vocational Training Center – Makassar",
          "Don Bosco Youth Center – Bandung",
          "Don Bosco Institute – Yogyakarta",
        ],
        image: "/indonesiamap.png",
      },
    ],
  },

  thailand: {
    country: "thailand",
    centers: [
      {
        schools: [
          "Don Bosco Technological College – Bangkok",
          "Don Bosco Vitthaya School",
          "Don Bosco Technological College – Banpong",
          "Don Bosco Skills Development Center – Chiang Mai",
        ],
        image: "/thailand-map.png",
      },
    ],
  },

  cambodia: {
    country: "cambodia",
    centers: [
      {
        schools: [
          "Don Bosco Technical School – Phnom Penh",
          "Don Bosco Technical School – Sihanoukville",
          "Don Bosco Hotel School – Kep",
          "Don Bosco Vocational Training Center – Battambang",
        ],
        image: "/cambodia-map.png",
      },
    ],
  },

  timorleste: {
    country: "timorleste",
    centers: [
      {
        schools: [
          "Don Bosco Youth Center – Comoro Dili",
          "Don Bosco Technical School – Fatumaca",
        ],
        image: "/timorleste-map.png",
      },
    ],
  },


  laos: {
    country: "Laos",
    centers: [
      {
        schools: [
          "Don Bosco Youth Vocational Training Center – Vientiane"
        ],
        image: "/laos-map.png",
      },
    ],
  },

  myanmar: {
    country: "Myanmar",
    centers: [
      {
        schools: [
          "Don Bosco Vocational Training Center – Myitkyina",
          "Don Bosco Youth Center – Mandalay",
          "Don Bosco Vocational Training Center – Yangon"
        ],
        image: "/myanmar-map.png",
      },
    ],
  },

  vietnam: {
    country: "Vietnam",
    centers: [
      {
        schools: [
          "Don Bosco Ky Anh Private Vocational Training Center",
         "Don Bosco Dong Thuan Private Vocational Training Center",
          "Don Bosco Technical College – Phuoc Loc",
          "Don Bosco Vocational School – Tan Tien",
          "Don Bosco Hospitality Training Center – An Re Mai Sen",
          "Don Bosco Vocational Intermediate School – Da Nang"
        ],
        image: "/vietnam-map.png",
      },
    ],
  },
};